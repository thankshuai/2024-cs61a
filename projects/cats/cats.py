...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def h(s):
        worlds = split(lower(remove_punctuation(s)))
        for world in worlds:
            if world in subject:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    sub_world =[diff_function(typed_word, world, limit) for world in word_list]
    if min(sub_world, key = abs) > limit or typed_word in word_list :
        return typed_word
    else:
        for i in range(len(sub_world)):
            if abs(sub_world[i]) == min(sub_world, key = abs):
                return word_list[i]
    # END PROBLEM 5
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(min(len(typed), len(source))):
        if typed[i] == source[i]:
            ...
            break
    total = total / lens
    upload({'id': user_id, 'progress': total})
    return total
    # END PROBLEM 8
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_list = [[] for _ in range(len(player_indices))]
    for i in word_indices:
        ...
        for j in player_indices[1:]:
            if time(match, j, i) < min_time:
                ...
                x = j
        fastest_list[x].append(get_word(match, i))
    return fastest_list
